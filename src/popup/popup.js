  ...
  .then((tabs) => {
    ...
    chrome.storage.session.get("tab" + tab.id).then((data) => {
      try {
          ...
          fetch(
            ...
            response.json().then(function (resData) {
              if (resData.green == true) {
                ...
                document.getElementById("greenBar").style.background =
                  "rgba(142, 202, 46, 0.5)";
                document.getElementById("greenBar").style.border =
                  "1px solid green";
        ...
        document.getElementById("importValBytes").innerHTML =
          x.importedFontSizeLab;
        document.getElementById("otherValBytes").innerHTML =
          x.otherTransSizeLab;
        ...
        document.getElementById("Lab1ValPercent").innerHTML =
          x.num1TransSizePercent;
        document.getElementById("Lab2ValPercent").innerHTML =
          x.num2TransSizePercent;
        document.getElementById("Lab3ValPercent").innerHTML =
          x.num3TransSizePercent;
        ...
        switch (x.finalScore >= 0) {
          case x.finalScore >= 92:
            ...
            var barColors = ["#32a852", "#f2f2f2"];
            break;

          case x.finalScore >= 78:
            var ctx = document.getElementById("myChart").getContext("2d");
            var yValues = [x.finalScore, scoreDiff];
            var barColors = ["#8ECA2E", "#f2f2f2"];
            break;

          case x.finalScore >= 70:
            var ctx = document.getElementById("myChart").getContext("2d");
            var yValues = [x.finalScore, scoreDiff];
            var barColors = ["#ebdc3d", "#f2f2f2"];
            break;
        ...
        var co2_textMessage = (co2Total / 0.014).toPrecision(3);
        ...
        var co2_3g = [
          "About the amount of CO2 released by a can of pop",
          "It takes 1 mature tree 1.5 hours to absorb the CO2 emitted on page load",
          "It would take 6 minutes of human breathing to emit the same amount of CO2",
          "Same amount of CO2 released as burning a candle for " +
            co2_candle +
            " minutes",
          "Same amount of CO2 released as using a plastic lighter for " +
            co2_lighter +
            " seconds.",
          "Sending " +
            co2_textMessage +
            " text messages takes the same amount of CO2 as loading this page.",
        ];

        var co2_2g = [
          "Amount of CO2 released as burning 1/3 of a charcoal briquette",
          "About the amount of CO2 released by a can of pop",
          "It would take 4 minutes of human breathing to emit the same amount of CO2",
          "It would take 1 mature tree 1 hour to absorb the CO2 emitted*",
          "Same amount of CO2 released as burning a candle for " +
            co2_candle +
            " minutes",
          "Same amount of CO2 released as using a plastic lighter for " +
        ...
        var co2_1g = [
          "It would take 2 minutes of human breathing to emit the same amount of CO2",
          "It takes 1 mature tree 30 minutes to absorb the CO2 emitted on page load",
          "Same amount of CO2 released as burning a candle for " +
            co2_candle +
            " minutes.",
          "Same amount of CO2 released as using a plastic lighter for " +
        ...
        var co2_halfg = [
          "It would take 1 minutes of human breathing to emit the same amount of CO2.",
          "It takes 1 mature tree 15 minutes to absorb the CO2 emitted on page load.",
          "Same amount of CO2 released as burning a candle for " +
            co2_candle +
            " minutes.",
          "Same amount of CO2 released as burning a plastic lighter for " +
            co2_lighter +
            " seconds.",
          "Sending " +
            co2_textMessage +
        ...
        var kwh_LEDLightBulb = (kwhTotal / 0.0000023).toPrecision(3);

        var kwh_LEDLightBulb = kwh_LEDLightBulb.toString();

        var kwh_MicrowaveSec = (kwhTotal / 0.0001).toPrecision(2);

        var kwh_MicrowaveSec = kwh_MicrowaveSec.toString();

        var kwh_MiniFridgeSec = (kwhTotal / 0.00001).toPrecision(2);

        var kwh_MiniFridgeSec = kwh_MiniFridgeSec.toString();

        var kwh_AAABattery = Math.round((kwhTotal / 0.0018) * 100);

        var kwh_AAABattery = kwh_AAABattery.toString();

        var kwh_OneCalorie = Math.round(kwhTotal / 0.001);

        var kwh_OneCalorie = kwh_OneCalorie.toString();

        var kwh_CellPhoneCharge = Math.round((kwhTotal / 0.006) * 100);

        var kwh_CellPhoneCharge = kwh_CellPhoneCharge.toString();

        var kwh_LCDTVCharge = (kwhTotal / 0.00003).toPrecision(2);

        var kwh_LCDTVCharge = kwh_LCDTVCharge.toString();

        var kwh_CeilingFan = (kwhTotal / 0.000015).toPrecision(2);

        var kwh_CeilingFan = kwh_CeilingFan.toString();

        var kwh_ChristmasLights = (kwhTotal / 0.000036).toPrecision(2);

        var kwh_ChristmasLights = kwh_ChristmasLights.toString();

        var kwh_TurnSignal = Math.round(kwhTotal / 0.00008);
        ...
        switch (kwhTotal >= 0) {
          case co2Total > 3:
            if (co2SelectorMoreThan3 == 0) {
              document.getElementById("actualFact").innerHTML =
                "It would take 1 mature tree " +
                treeAbsorb +
                " hours to absorb the CO2 emitted from page load.";
              ...
              document.getElementById("actualFact").innerHTML =
                "It would take " +
                co2_breath +
                " minutes of human breathing to emit the same amount of CO2.";
            }
          ...
          case kwhTotal > 0.001:
            if (kwhMoreThan001 == 0) {
              document.getElementById("actualFact").innerHTML =
                "Loading this page takes the same amount of energy as charging your phone " +
                kwh_CellPhoneCharge +
                "%.";
            } else if (kwhMoreThan001 == 1 && kwhTotal <= 0.0018) {
              document.getElementById("actualFact").innerHTML =
                "Same amount of energy on page load as using " +
                kwh_AAABattery +
                "% of a AAA battery.";
            } else if (kwhMoreThan001 == 2) {
              document.getElementById("actualFact").innerHTML =
                "The energy required to run a ceiling fan " +
                kwh_CeilingFan +
                " seconds.";
            } else if (kwhMoreThan001 == 3) {
              document.getElementById("actualFact").innerHTML =
                "Energy used to power a TV screen for " +
                kwh_LCDTVCharge +
                " seconds.";
            } else if (kwhMoreThan001 == 4) {
              document.getElementById("actualFact").innerHTML =
                "Loading this page could power a 25ft strand of Christmas lights for " +
                kwh_ChristmasLights +
                " seconds.";
            } else if (kwhMoreThan001 == 5) {
              document.getElementById("actualFact").innerHTML =
                "The energy to load this page could power a car's turn signal " +
                kwh_TurnSignal +
                " times.";
            } else {
              document.getElementById("actualFact").innerHTML =
                kwh_Cal +
                " Calorie(s), about the same energy emitted as walking " +
                amountOfSteps +
                " steps.";
            }
          ...
          case kwhTotal > 0.0002:
            if (kwhMoreThan0002 == 0) {
              ...
              document.getElementById("actualFact").innerHTML =
                "Loading this pages takes the same amount of energy as " +
                kwh_MicrowaveSec +
                " seconds in a microwave.";
            } else if (kwhMoreThan0002 == 3) {
              document.getElementById("actualFact").innerHTML =
                "Loading this pages takes the same amount of energy as running a mini fridge " +
                kwh_MiniFridgeSec +
                " seconds.";
          ...
          case kwhTotal >= 0:
            if (kwhMoreThan0 == 0) {
              document.getElementById("actualFact").innerHTML =
                kwh_IncanLightBulb +
                " seconds running an incandescent lightbulb.";
              ...
              document.getElementById("actualFact").innerHTML =
                kwh_LEDLightBulb + " seconds running an LED lightbulb.";
            }
        ...
        switch (lazyLoadChart >= 0) {
          case lazyLoadChart == 110:
            ...
            document.getElementById("lazyBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("lazyBar").style.border = "1px solid green";

            break;
          case lazyLoadChart == 100:
            document.getElementById("lazyBar").innerHTML =
              x.lazyLoadChart + "%";
            document.getElementById("lazyBar").style.width = "240px";
            document.getElementById("lazyBar").style.background =
              "rgba(142, 202, 46, 0.5)";
          ...
          case lazyLoadChart >= 55:
            ...
            document.getElementById("lazyBar").style.background =
              "rgba(235, 220, 61, 0.5)";
            document.getElementById("lazyBar").style.border =
              "1px solid yellow";
        ...
        switch (svgChart >= 0) {
          case svgChart == 110:
            ...
            document.getElementById("svgBar").style.background =
              "rgba(142, 202, 46, 0.5)";
        ...
        switch (decodedBodySizeChart >= 0) {
          case decodedBodySizeChart <= 150000:
            ...
            document.getElementById("byteSizeBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("byteSizeBar").style.border =
              "1px solid green";
        ...
        switch (loadTimeChart >= 0) {
          case loadTimeChart <= 1:
            ...
            document.getElementById("loadBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("loadTable").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("loadBar").style.border = "1px solid green";
            document.getElementById("num1LongNameID").innerHTML =
              x.num1LoadName;
            document.getElementById("num2LongNameID").innerHTML =
              x.num2LoadName;
            document.getElementById("num3LongNameID").innerHTML =
              x.num3LoadName;
          ...
          case loadTimeChart <= 4:
            ...
            document.getElementById("loadTable").style.background =
              "rgba(235, 220, 61, 0.5)";
            document.getElementById("loadBar").style.border =
              "1px solid yellow";
            document.getElementById("num1LongNameID").innerHTML =
              x.num1LoadName;
            document.getElementById("num2LongNameID").innerHTML =
        ...
        switch (jsChart >= 0) {
          case jsChart <= 10000000:
            ...
            document.getElementById("jsBar").style.background =
              "rgba(142, 202, 46, 0.5)";
        ...
        switch (importChart >= 0) {
          case importChart == 0:
            ...
            document.getElementById("fontBar").style.background =
              "rgba(142, 202, 46, 0.5)";
          ...
          case importChart == 1:
            ...
            document.getElementById("fontBar").style.background =
              "rgba(241, 137, 49, 0.5)";
            document.getElementById("fontBar").style.border =
              "1px solid orange";
        ...
        switch (htmlChart >= 0) {
          case htmlChart <= 200001:
            ...
            document.getElementById("htmlBar").style.background =
              "rgba(142, 202, 46, 0.5)";
          ...
          case htmlChart <= 500001:
            ...
            document.getElementById("htmlBar").style.background =
              "rgba(235, 220, 61, 0.5)";
            document.getElementById("htmlBar").style.border =
              "1px solid yellow";
        ...
        switch (transferSizeChart >= 0) {
          case transferSizeChart == 0:
            ...
            document.getElementById("transferBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("transferBar").style.border =
              "1px solid green";
            document.getElementById("num1TransSizeLabID").innerHTML =
              x.num1TransSizeLab;
            document.getElementById("num2TransSizeLabID").innerHTML =
              x.num2TransSizeLab;
            document.getElementById("num3TransSizeLabID").innerHTML =
              x.num3TransSizeLab;
            document.getElementById("num1TransNameID").innerHTML =
              x.num1TransName;
            document.getElementById("num2TransNameID").innerHTML =
              x.num2TransName;
            document.getElementById("num3TransNameID").innerHTML =
              x.num3TransName;
            document.getElementById("jsTableVal").innerHTML = x.jsSizeLab;
            document.getElementById("imgTableVal").innerHTML =
              x.imgTransSizeLab;
            document.getElementById("cssTableVal").innerHTML = x.CSSSizeLab;
            document.getElementById("importTableVal").innerHTML =
              x.importedFontSizeLab;
            document.getElementById("otherTableVal").innerHTML =
              x.otherTransSizeLab;
            break;
        ...
        switch (resImgChart >= 0) {
          case resImgChart == 110:
            document.getElementById("resImgBar").innerHTML =
              "No Images on Page";
            document.getElementById("resImgBar").style.width = "240px";
            document.getElementById("resImgBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("resImgBar").style.border =
              "1px solid green";
            break;
        ...
        switch (intStyleSheet >= 0) {
          case intStyleSheet == 0:
            ...
            document.getElementById("intStyleBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("intStyleBar").style.border =
              "1px solid green";
            break;
        ...
        switch (numStyleSheet >= 0) {
          case numStyleSheet == 0:
            ...
            document.getElementById("numStyleBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("numStyleBar").style.border =
              "1px solid green";
            document.getElementById("extStyleTextA").innerHTML =
              x.styleSheetSources;
          ...
          case x.cssTransLabel === "All style sheets cached!":
            ...
            document.getElementById("extStyleHeader").innerHTML =
              "External Style Sheet Sources: " + "All style sheets cached!";
            document.getElementById("extSSDiv").style.background =
              "rgba(142, 202, 46, 0.5)";
          ...
          case numStyleSheet == 1:
            ...
            document.getElementById("extStyleTextA").style.display = "block";
            document.getElementById("extStyleHeader").innerHTML =
              "External Style Sheet Sources: " + numStyleSheet;
            document.getElementById("extSSDiv").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("extStyleTextA").rows = "3";
            break;
        ...
        switch (redirects >= 0) {
          case redirects == 0:
            ...
            document.getElementById("redirectBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("redirectBar").style.border =
              "1px solid green";
        ...
        switch (cookieLen >= 0) {
          case cookieLen == 1:
            ...
            document.getElementById("cookieBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("cookieBar").style.border =
              "1px solid green";
            document.getElementById("cookDiv").style.background =
              "rgba(142, 202, 46, 0.5)";
          ...
          case cookieLen == 2:
            ...
            document.getElementById("cookieHeader").innerHTML =
              "Cookies Found:";
        ...
        switch (emptyURL >= 0) {
          case emptyURL == 0:
            ...
            document.getElementById("srcBar").style.background =
              "rgba(142, 202, 46, 0.5)";
        ...
        switch (cacheNum >= 0) {
          case cacheNum >= 31536000:
            document.getElementById("cacheBar").innerHTML =
              x.cacheDays + " days, " + x.cacheHours + " hrs";
            document.getElementById("cacheBar").style.width = "240px";
            document.getElementById("cacheBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("cacheBar").style.border =
              "1px solid green";
            break;
          ...
          case cacheNum >= 36000:
            document.getElementById("cacheBar").innerHTML =
              x.cacheHours + " hrs, " + x.cacheMinutes + " min";
          ...
          case cacheNum >= 600:
            document.getElementById("cacheBar").innerHTML =
              x.cacheMinutes + " min, " + x.cacheSeconds + " sec";
        ...
        switch (colorScore) {
          case "black":
            ...
            document.getElementById("colorBar").style.background =
              "rgba(142, 202, 46, 0.5)";
            document.getElementById("colorBar").style.border =
              "1px solid green";
            break;
          case "black/grey":
            document.getElementById("colorBar").innerHTML = colorScore;
...
if (selectedTab) document.getElementById(selectedTab).click();
else document.getElementById("factButton").click();
...
document.getElementById("htmlButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if page content was simplified.";
...
document.getElementById("LLButton").onclick = function (evt) {
  ...
  document.getElementById("m2").innerHTML =
    'With "lazy loading", files are only loaded when they reach the visible area of the browser window, or shortly before it. Modern browsers allow to insert loading=“lazy“ as an attribute in the img element. Lazy loading is useful because it allows images lower on a page to not be loaded if it\'s not in immediate view. Looking for the following Lazy-Loading methods: loading="lazy", class="lozad", class="b-lazy", class="lazyloaded", and class="lazy".';
...
//    document.getElementById("m4").innerHTML = "Grade increases to " + lazyLoadScore + " if all images were Lazy Loaded";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if > 65% of images were lazy loaded";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("imgButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if image types were changed to either svg, webp, or avif.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("importButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Imported Font Files cause an unnecessary amount of data loading on your page. Try to just use system fonts. If your imported Font file is .TFF it’s a less efficient font file format than .WOFF. Stick to modern web font file formats: WOFF and WOFF2, these use higher compression methods compared to TTF, SVG and OFT file formats. For imported fonts try to only include the characters needed for your website. Some font files will have distiction between styling \(i.e bolding). If styling is not being used, try removing this from the font file.";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if system fonts were used.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("resImgButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if > 70% of images were responsive by screen size.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("intSSButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Make sure CSS files are separate from page\’s HTML code. If CSS is in the HMTL body then the code must be sent for each page request- increasing the amount of data sent. If CSS is in separate files, the browser can store them in local cache.";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if there were < 3 internal style sheets \(style=). ";
...
document.getElementById("fileSSButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Reducing unnecessary files will help with overall page size. Reducing the number of CSS files reduces the number of HTTP requests that are required on page load. Ask yourself, can I just combine my multiple Style Sheets into one?";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if there were < 3 style sheet files.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("pageLoadButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Reducing your page load time can improve the energy required to render pages. Decreasing page load time can improve your SEO; which enables users to find the content they're looking for- faster. Google Search prioritizes pages with faster page load time. Decreasing load time improves the user experience for users with slower connection speeds.";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if page load time is decreased to less than 2 seconds.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("pageSizeButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Overall page size, is total amount of data required to load the page. This is the amount of data required to load your page for each unique user for the first time. It can also be thought of as the amount of data uploaded to servers.";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if overall page size was decreased.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("transferButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if transfer size was decreased.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("jsButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou && mayYou !== "undefined") {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if less JavaScript was executed on page.";
...
document.getElementById("hostButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    'Green Hosted is powered by the Green Web Foundation who defines the dataset as: "the largest dataset in the world of which sites use renewable power." Powering your site with renewables saves energy from energy produced by fossil fuels.';
...
document.getElementById("redirectButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Redirects are an unnecessary use of resources. It causes additional data to be loaded.";
  ...
  if (may4You !== mayYou && mayYou !== "undefined") {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " ,if there were no redirects.";
...
document.getElementById("cookieButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    "Cookies are sent with every request, so they can worsen performance (especially for mobile data connections). Are all these cookies on your page necessary? Do you know the specific purpose of each cookie on your page? ";
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if there were < 4 cookies set on page.";
...
document.getElementById("emptyButton").onclick = function (evt) {
  ...
  //document.getElementById('LLButton').style.background = 'rgba(241, 137, 49, .5)';
  document.getElementById("m2").innerHTML =
    'Having an empty href link is bad practice. Browser will call the directory in which the page is located if SRC Attribute is empty. This results in additional HTTP requests. For example, if http://www.test.com/ has script src="" it will make a request to all / in all browsers to the homepage of the domain. Doubling the traffic to your homepage.';
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if there were no empty 'src' tags.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("cacheButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " + mayYou + " if cache time is > 365 days.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
...
document.getElementById("colorButton").onclick = function (evt) {
  ...
  if (may4You !== mayYou) {
    document.getElementById("mLL").innerHTML =
      "Grade increases to " +
      mayYou +
      " if website background color was black / dark gray.";
    document.getElementById("mLL").style.display = "block";
  } else {
    document.getElementById("mLL").style.display = "none";
  }
